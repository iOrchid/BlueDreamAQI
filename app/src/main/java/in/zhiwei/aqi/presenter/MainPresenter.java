...
public class MainPresenter implements IAQIContract.IAQIPresenter {

	private IAQIContract.IAQIView mAQIView;//AQIView的对象
	private String apkUrl;//apk下载url
	private String updateInfo;//升级信息
	private Disposable disposable;//请求aqi的retrofit的网络控制阀
	private Disposable disposableUpgrade;//检查升级的disposable
	 ...
	 */
	public MainPresenter(IAQIContract.IAQIView view) {
		this.mAQIView = view;
	}
	...
	public void checkUpgrade(@NonNull Context context) {
		...
		//请求网络，获得返回json
		disposableUpgrade = HttpApi.getInstance().create(AQIService.class)
				.checkUpgrade()
				.subscribeOn(Schedulers.io())
				.observeOn(AndroidSchedulers.mainThread())
